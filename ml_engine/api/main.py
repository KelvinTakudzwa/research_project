...
def predict_anomaly(data: SensorData, _=Depends(verify_service_token)):
    with model_lock:
        ...
        # ── Stage 1: Isolation Forest gate ───────────────────────────────────
        # Strict hierarchy: if IF says Normal, stop here — RF never runs.
        # This preserves the two-stage pipeline thesis and saves CPU cycles.
        try:
            if_pred  = if_model.predict(X)[0]
            if_score = float(if_model.decision_function(X)[0])
        except Exception as e:
            print(f"[ML_API] IF model error: {e}")
            raise HTTPException(status_code=503, detail="Models are stale — retrain in progress.")

        is_outlier = (if_pred == -1)

        if not is_outlier:
            # IF gate closed: no anomaly detected, return immediately
            return {
                "status":        "Normal",
                "soh_percent":   100.0,
                "confidence":    1.0,
        ...
        # ── Stage 2: RF classifier — only reached when IF flagged anomaly ────
        try:
            ...
            print(f"[ML_API] RF model error (retrain needed): {e}")
            raise HTTPException(status_code=503, detail="Models are stale — retrain in progress.")

    # RF decision: conf ≥ 0.50 → specific fault; below threshold → Uncertain_Anomaly
    if rf_pred_class != 'Normal' and confidence >= 0.50:
        final_status      = rf_pred_class
        reported_confidence = round(confidence, 4)
    else:
        final_status = "Uncertain_Anomaly"
        # RF confidence is meaningless here (it voted Normal, but IF disagrees).
        # Report IF deviation severity instead: abs(if_score) × 2 → [0.0, 1.0].
        # A score near 0 = barely outside normal; near 1 = extreme outlier.
        reported_confidence = round(min(abs(if_score) * 2, 1.0), 4)
